...
const Referral: FC = () => {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row gap-10 lg:gap-5">
      ...
      <MyReferralLink address={wallet?.address} />
    </div>
  )
}

const MyReferralLink: FC<{ address?: string }> = ({ address }) => {
  const { wallet } = useWeb3()
      ...
      if (wallet?.address) {
        ...
        try {
          const res = await axios.get(
            'http://wcfi.wii.camp/v1.0/refs/commission',
            { params },
          )
          setTotal(getResponseData(res) || 0)
        } catch (err) {

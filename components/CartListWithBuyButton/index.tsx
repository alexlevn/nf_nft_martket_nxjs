          ...
          <ModalTrigger
            ...
            renderChildren={(closeModal) => (
              <NFTDetail
                ...
                renderAction={() =>
                  item.listingId ? (
                    <ButtonBuyNft listingId={item.listingId} />
                  ) : null
                }
              />
            )}
...
const ButtonBuyNft: FC<{ listingId: string }> = ({ listingId }) => {
  const { connected, connect, allowanceMK, approve, buyToken } = useWeb3()
  ...
  return connected() === false ? (
    ...
    <>
      ...
      <ButtonGradient
        className="py-2 text-base"
        onClick={() => buyToken(listingId, callbackBuy)}
      >
        Buy
      </ButtonGradient>
      {/* {renderSpinner()} */}
...
// <ButtonGradient onClick={() => console.log('Buy')}>Buy</ButtonGradient>
